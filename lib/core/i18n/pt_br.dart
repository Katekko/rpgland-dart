...
class TranslationPtBr extends CommandTranslations {
  ...
  TranslationPtBr(this.commandChar) : super();
  ...
  Map<String, dynamic> get commands => {
        ...
        'shop': {
          'info': (List<ItemModel> items) {
            final itemLines = items
                .map((item) =>
                    '🛒 *${item.name}* - Preço: *${item.price}* moedas')
                .toList();
        ...
        'heal': {
          'healedWithItem': (int healedAmount, int currentHealth,
                  String itemName) =>
        ...
        'language': {
          'changed': (String lang) =>
              '✅ Idioma alterado com sucesso para *$lang*',
          'error':
